...
public class SocketChannelChatServer {
    ...
    private static void doRead(SelectionKey selectionKey) throws IOException {
        ...
        if (read == -1) { // if connection is closed by the client
            ...
            if ("bye".equals(message.toString().trim())) {
                byteBuffer.rewind();
                sendMessageToAll(socketChannel, byteBuffer);
                doClose(socketChannel);
            } else {
                byteBuffer.rewind();
    ...
    private static void doWrite(SelectionKey selectionKey) {
        ...
        while (!pendingData.isEmpty()) { // start sending to client from queue
            ...
            sendMessageToAll(socketChannel, buf);
        }
    ...
    private static void createResponse(SocketChannel socketChannel, ByteBuffer buf, SocketChannel client) {
        try {
            buf.rewind();
            ByteBuffer response = ByteBuffer.allocate(1024).put(("[" + socketChannel.getRemoteAddress() + "]: ").getBytes()).put(buf);
            response.rewind();
